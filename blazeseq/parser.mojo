...
struct CoordParser:
    ...
    fn _parse_record(inout self) raises -> RecordCoord:
        ...
        if self.stream.buf[self.stream.map_pos_2_buf(line1.start.value())] != read_header:
            raise Error("Sequence Header is corrupt")

        var line2 = self.stream.read_next_coord()

        var line3 = self.stream.read_next_coord()
        if self.stream.buf[self.stream.map_pos_2_buf(line3.start.value())] != quality_header:
